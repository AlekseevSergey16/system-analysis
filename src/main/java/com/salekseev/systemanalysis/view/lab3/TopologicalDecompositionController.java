...
public class TopologicalDecompositionController {

    private final TopologicalDecompositionView view;
    private final Graph graph = new Graph();

    public TopologicalDecompositionController(TopologicalDecompositionView view) {
        this.view = view;
        this.initialize();
    }
    ...
    private void calculateButtonOnAction(ActionEvent event) {
        clearPane();
        List<Subgraph> subgraphList = graph.setAdjacencyMatrix(Main.lab3).topologicalDecomposition();
        for (int i = 0; i < subgraphList.size(); i++) {
            view.subgraphListView.getItems()
                    .add("Сильно связный подграф (%d) = ".formatted(i + 1) +
                            subgraphList.get(i).V.stream()
                                    .map(v -> v + 1)
        ...
        Integer[][] adjacencyMatrix = graph.setOriented(true).buildAdjacencyMatrix(subgraphList);
    ...
    private void clearPane() {
        ...
        view.subgraphListView.getItems().clear();
    }
